...
class VispipelineProjection(BaseEstimator, TransformerMixin):
    ...
    def _send_data(self, X, y):
        ...
        self.tmp_file = tempfile.NamedTemporaryFile(mode='w', dir=self.tmp_dir.name, suffix='.data', delete=False)
    ...
    def _receive_data(self):
        ...
        if len(proj_files) != 1:
            raise ValueError('Error looking for projection file inside %s' % self.tmp_dir.name)
    ...
    def _run(self, X, y, cmdargs):
        ...
        cmdline = [self.command, self.projection, self.tmp_dir.name] + [str(x) for x in cmdargs]

        if self.verbose:
            print('#################################################')
            print(' '.join(cmdline))

        rc = subprocess.run(cmdline, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=3600, check=True)
...
class PLMP(VispipelineProjection):
    ...
    def __init__(self, command='vp-run',
                      ...
                      verbose=False):
        super(PLMP, self).__init__(projection='plmp', command=command, verbose=verbose)
    ...
    def set_params(self, command='vp-run',
        ...
        try:
            self.dissimilarity_type_index = self.dissimilarity_types.index(dissimilarity_type)
        except:
...
#    7. Min Moving Euclidean
    def __init__(self, command='vp-run', 
                 ...
                 dissimilarity_type='euclidean',
                 verbose=False):
        super(IDMAP, self).__init__(projection='idmap', command=command, verbose=verbose)
...
#    7. Min Moving Euclidean
    def __init__(self, command='vp-run', 
                 ...
                 dissimilarity_type='euclidean',
                 verbose=False):
        super(LSP, self).__init__(projection='lsp', command=command, verbose=verbose)
    ...
    def set_params(self, command='vp-run', 
        ...
        try:
            self.control_point_type_index = self.control_point_types.index(control_point_type)
        except:
...
#    7. Min Moving Euclidean
    def __init__(self, command='vp-run',
                 ...
                 dissimilarity_type='euclidean',
                 verbose=False):
        super(PLSP, self).__init__(projection='plsp', command=command, verbose=verbose)
...
#    4. SPAM
    def __init__(self, command='vp-run',
                 ...
                 sample_type='random',
                 verbose=False):
        super(LAMP, self).__init__(projection='lamp', command=command, verbose=verbose)
...
class ProjectionByClustering(VispipelineProjection):
    ...
    def __init__(self, command='vp-run', 
                 ...
                 cluster_factor=4.5,
                 verbose=False):
        super(ProjectionByClustering, self).__init__(projection='projclus', command=command, verbose=verbose)
...
class LandmarkIsomap(VispipelineProjection):
    ...
    def __init__(self, command='vp-run',
                 ...
                 dissimilarity_type='euclidean',
                 verbose=False):
        super(LandmarkIsomap, self).__init__(projection='lisomap', command=command, verbose=verbose)
