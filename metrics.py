...
# TODO: test Normalized Stress metric
...
def eval_dc_metrics(**kwargs):
    ...
    DISTANCES['D_high_list'] = '%s/%s_D_high_list.npy' % (output_dir, dataset_name)
    DISTANCES['D_high_matrix'] = '%s/%s_D_high_matrix.npy' % (output_dir, dataset_name)
...
def eval_pq_metrics(**kwargs):
    ...
    DISTANCES[id_run]['D_low_list'] = '%s/%s_D_low_list_%s.npy' % (output_dir, dataset_name, clean_id_run)
    DISTANCES[id_run]['D_low_matrix'] = '%s/%s_D_low_matrix_%s.npy' % (output_dir, dataset_name, clean_id_run)
    ...
    DISTANCES[id_run]['Q'] = '%s/%s_Q_%s.npy' % (output_dir, dataset_name, clean_id_run)
...
def results_to_dataframe(results, dataset_name):
    ...
    df = df.loc[:, ['dataset_name', 'projection_name', 'projection_parameters'] + list(column_list)]
...
def metric_coranking_matrix(D_high, D_low):
    ...
    Q, _, _ = np.histogram2d(high_rank.flatten(), low_rank.flatten(), bins=N_SAMPLES)
    Q = Q[1:, 1:]
...
def metric_pq_shepard_diagram_correlation(id_run, dataset_name):
    ...
    r = stats.spearmanr(D_high, D_low)[0]
